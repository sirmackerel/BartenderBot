...
prominent and obvious device in the teleimmersive operation system. ROS has an existing Kinect libraries that allows
...
for a learning planner. Current planners are very slow to calculate, but
(TODO, mikephil?) have already demonstrated significant improvements in
efficiency by creating a planner which uses paths it has already computed
as a basis whenever it is requested to find a new path. If it could instead be
made to learn from recorded teleoperation data,
it is possible could learn more 'natural' motions for and paths for common
tasks.
